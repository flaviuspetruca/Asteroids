...
newPosEnemies hd enemies (e@(Spaceship s p or b):es) (x1,y1) bs lives c r o (x,y)
  ...
  | otherwise = (fst $ newPosEnemies hd enemies es (x1,y1) bs lives c r o (x,y), newLives-1)
    where dss | s == Large  = 40
          ...
          newLives = snd (newPosEnemies hd enemies es (x1,y1) bs lives c r o (x,y))
newPosEnemies hd enemies (e@(Asteroid  s p or):es) (x1,y1) bs lives c r o (x,y)
  | notCollisionEB e bs == False || notCollisionAsSs e (getSpaceships enemies) == False
    = case  () of
            ()  | s /= Small -> (createSmallerAsteroids s p o ++ fst (newPosEnemies hd enemies es (x1,y1) bs lives c r o (x,y)), newLives)
                | otherwise  -> newPosEnemies hd enemies es (x1,y1) bs lives c r o (x,y)
  | distance2 (x1,y1) (newPos p or 1.5) > ds || hd = (f e : fst (newPosEnemies hd enemies es (x1,y1) bs lives c r o (x,y)), newLives)
  | otherwise
    = case  () of
    ...
    where f (Asteroid s p or)
          ...
          newLives | playerHit enemyBullets && not hd = lives - 1
                   | otherwise = snd (newPosEnemies hd enemies es (x1,y1) bs lives c r o (x,y))

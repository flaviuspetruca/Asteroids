...
render (MkGameState ks (MkPlayer _ _ im (x,y) _ l o oo) en b _ _ _ _ )
    | l > 0 = pictures (mkPlayer im white x y o:(enemies++bullets))
    | otherwise = pictures [deadText]
...
update sec g@(MkGameState ks (MkPlayer n gs im (x,y) vel l o oo) e a d st p r)
  | SpecialKey KeySpace `elem` ks && Char 'w' `elem` ks && Char 'd' `elem` ks
  = case  () of
          () | oo/=o && vel > 0 -> movePlayer sec (MkGameState newKeys (MkPlayer n updatedGs fire (cgPlPos oo) (acc vel (-0.05)) updatedLives (newOr o (-3)) (newOr o   3)) npe (npb bullets e gs) d st p r)
             | otherwise        -> movePlayer sec (MkGameState newKeys (MkPlayer n updatedGs fire (cgPlPos oo) (acc vel 0.3) updatedLives (newOr o (-3)) (newOr o   3)) npe (npb bullets e gs) d st p r)
  ...
  | Char 'w' `elem` ks && Char 'd' `elem` ks
    = if oo /= o && vel > 0
        then movePlayer sec (MkGameState ks (MkPlayer n updatedGs fire (cgPlPos oo) (acc vel (-0.05)) updatedLives (newOr o (-3)) (newOr o   3)) npe (npb a e gs) d st p r)
        else movePlayer sec (MkGameState ks (MkPlayer n updatedGs fire (cgPlPos oo) (acc vel 0.3) updatedLives (newOr o (-3)) (newOr o   3)) npe (npb a e gs) d st p r)
  ...
  | SpecialKey KeySpace `elem` ks && Char 'w' `elem` ks
    = case  () of
            () | oo/=o && vel > 0 -> movePlayer sec (MkGameState newKeys  (MkPlayer n updatedGs fire (cgPlPos o) (acc vel (-0.1)) updatedLives o o) npe (npb bullets e gs) d st p r)
               | otherwise        -> movePlayer sec (MkGameState newKeys  (MkPlayer n updatedGs fire (cgPlPos o) (acc vel 0.3) updatedLives o o) npe (npb bullets e gs) d st p r)

  | SpecialKey KeySpace `elem` ks && Char 'a' `elem` ks
    = movePlayer sec (MkGameState newKeys  (MkPlayer n updatedGs False (cgPlPos oo) (acc vel (-0.07)) updatedLives (newOr o 3) oo) npe (npb bullets e gs) d st p r)
  | SpecialKey KeySpace `elem` ks && Char 'd' `elem` ks
    = movePlayer sec (MkGameState newKeys  (MkPlayer n updatedGs False (cgPlPos oo) (acc vel (-0.07)) updatedLives (newOr o (-3)) oo) npe (npb bullets e gs) d st p r)
  | SpecialKey KeySpace `elem` ks
    = movePlayer sec (MkGameState newKeys  (MkPlayer n updatedGs False (cgPlPos oo) (acc vel (-0.07)) updatedLives o oo) npe (npb bullets e gs) d st p r)

  | Char 'w' `elem` ks
    = if oo/=o && vel > 0
        then movePlayer sec (MkGameState ks (MkPlayer n updatedGs fire (cgPlPos o) (acc vel (-0.1)) updatedLives o o) npe (npb a e gs) d st p r)
        else movePlayer sec (MkGameState ks (MkPlayer n updatedGs fire (cgPlPos o) (acc vel 03) updatedLives o o) npe (npb a e gs) d st p r)
  | Char 'a' `elem` ks
    = if vel > 0
        then movePlayer sec (MkGameState ks (MkPlayer n updatedGs False (cgPlPos oo) (acc vel (-0.07)) updatedLives (newOr o   3) oo) npe (npb a e gs) d st p r)
        else movePlayer sec (MkGameState ks (MkPlayer n updatedGs False (cgPlPos oo) vel updatedLives (newOr o   5) oo) npe (npb a e gs) d st p r)
  | Char 'd' `elem` ks
    = if vel > 0
        then movePlayer sec (MkGameState ks (MkPlayer n updatedGs False (cgPlPos oo) (acc vel (-0.07)) updatedLives (newOr o (-3)) oo) npe (npb a e gs) d st p r)
        else movePlayer sec (MkGameState ks (MkPlayer n updatedGs False (cgPlPos oo) vel updatedLives (newOr o (-3)) oo) npe (npb a e gs) d st p r)
  | l == 0 = (MkMainMenu menuBox n gs)
  | otherwise = movePlayer sec (MkGameState ks (MkPlayer n updatedGs False (cgPlPos oo) (acc vel (-0.07)) updatedLives o oo) npe (npb a e gs) d st p r)
  where
...
handleKeys (EventKey (SpecialKey KeyEnter) _ _ _) en@(MkEnterName boxes currName)
  | length currName > 0 = (MkMainMenu updatedBoxes currName (-1))
...
handleKeys (EventKey (MouseButton LeftButton) Down _ mousePos) mm@(MkMainMenu boxes currName score)
  ...
  | otherwise = mm -- REMOVE?
    where updatedBoxes = makeText currName (-460) (80) : menuBox
          ...
          game = (MkGameState [] player enemies [] Easy True False sg)
          player = (MkPlayer currName 0 False (0,0) 0.0 3 90 90)
          enemies = fst $ mkAsteroids 5 (mkStdGen 2)
          sg = snd $ mkAsteroids 5 (mkStdGen 2)

handleKeys (EventKey key@(Char c) state _ _) g@(MkGameState ks (MkPlayer n gs im (x,y) vel l o oo) e a d st p r)
  ...
  | c == 'd' =  case state of Down  -> g { keys = insert key ks}
                              Up    -> g { keys = delete key ks}
handleKeys (EventKey key@(SpecialKey KeySpace) state _ _)  g@(MkGameState ks (MkPlayer n gs im (x,y) v l o oo) e a d st p r)
        | state == Down = case () of
...
handleKeys (EventKey (Char 'p') Down _ _) g@(MkGameState ks (MkPlayer n gs im (x,y) 0.0 l o oo) e a d st p r)
  = (MkPauseMenu g pauseBox)
handleKeys (EventKey (Char 'p') Down _ _) (MkPauseMenu g boxes)
  = g
handleKeys (EventKey (MouseButton LeftButton) Down _ mousePos) pm@(MkPauseMenu game@(MkGameState ks player@(MkPlayer n gs im (x,y) vel l o oo) e a d st p r) boxes)
  | x' > (-460) && x' < 500 && y' > (-60) && y' < 20 = game
